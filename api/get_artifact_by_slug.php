...
$slug = trim($_GET['slug'] ?? '');
if (!$slug) {
    abort('Slug fehlt.', 400);
}

// Nur alphanumerisch + Bindestriche erlauben
if (!preg_match('/^[a-z0-9\-]+$/', $slug)) {
    abort('Ungültiger Slug.', 400);
}

$db = get_db();

// 1. Artefakt per Slug laden
$stmt = $db->prepare("
    SELECT
        a.id,
        a.title,
...
$stmtP = $db->prepare("
    SELECT p.id, p.first_name, p.last_name, p.birth_year, p.death_year, p.portrait_url, ap.role
    FROM persons p
...
// 3. Weitere Orte
$stmtL = $db->prepare("
    SELECT l.id, l.name, l.slug, l.lat, l.lng, al.note
    FROM locations l
